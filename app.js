(function () {
  ...
  var els = {
    ...
    gameHammerClearBtn: document.getElementById("gameHammerClearBtn"),
    playHammerRow: document.getElementById("playHammerRow"),
    ...
    hammerPad: document.getElementById("hammerPad"),
    hammerPadTitle: document.getElementById("hammerPadTitle"),
    hammerOptions: document.getElementById("hammerOptions"),
    hammerPadClose: document.getElementById("hammerPadClose"),
  ...
  function teamHoleContestValue(teamKey, hole) {
    if (isQuota()) return teamHolePoints(teamKey, hole);
    return teamHoleScore(teamKey, hole);
  }

  function lowerScoreWinsHole() {
    return !isQuota();
  }

  function formatLeadText(aVal, bVal, lowerIsBetter, aLabel, bLabel) {
    if (aVal == null || bVal == null) return "—";
    if (aVal === bVal) return "AS";
    var aAhead = lowerIsBetter ? aVal < bVal : aVal > bVal;
    var margin = Math.abs(aVal - bVal);
    return (aAhead ? aLabel : bLabel) + "+" + margin;
  }

  function individualLeadMark(players, runs) {
    var bestId = null;
    var bestVal = null;
    var tied = false;
    players.forEach(function (p) {
      ...
      var better = isQuota() ? v > bestVal : v < bestVal;
      if (better) {
        bestVal = v;
        bestId = p.id;
        tied = false;
  ...
  function holePlayerResult(hole) {
    ...
    roundState.players.forEach(function (p) {
      ...
      scored += 1;
      var val = isQuota()
        ? holePoints(p, sc, hole)
        : playerUsedScore(p, sc, hole);
      if (val == null) return;
      if (best == null) {
        best = val;
        winners = [p.id];
      ...
      var better = isQuota() ? val > best : val < best;
      if (better) {
        best = val;
        winners = [p.id];
      } else if (val === best) winners.push(p.id);
    });
    if (!scored || scored < roundState.players.length)
      return { winnerIds: [], incomplete: true };
    if (winners.length !== 1)
      return { winnerIds: winners, incomplete: false, tie: true };
    return { winnerIds: winners, incomplete: false, tie: false };
  }
  ...
  function matchStatusSummary() {
    ...
    course.holes.forEach(function (h) {
      ...
      if (roundState.gameType === "hammer") {
        ...
        var ap = teamHolePoints("A", h);
        var bp = teamHolePoints("B", h);
        if (ap != null) runA += ap;
        if (bp != null) runB += bp;
      } else {
        var as = teamHoleScore("A", h);
        var bs = teamHoleScore("B", h);
        if (as != null) runA += as;
        if (bs != null) runB += bs;
      }
    });
    if (!played) return aLabel + " vs " + bLabel;
    var lead = formatLeadText(runA, runB, lowerBetter, aLabel, bLabel);
    return lead + " · thru " + played;
  }
  ...
  function updatePlayGameActions() {
    ...
    els.playHammerRow.hidden = !showHammer;
    if (showHammer) {
      if (!hamNow) {
        els.gameHammerBtn.textContent = "Hammer to 2x";
        els.gameHammerBtn.disabled = false;
      } else if (hamNow.multiplier >= MAX_HAMMER) {
        els.gameHammerBtn.textContent = "Hammer to " + MAX_HAMMER + "x";
        els.gameHammerBtn.disabled = true;
      } else {
        els.gameHammerBtn.textContent =
          "Hammer to " + hamNow.multiplier * 2 + "x";
        els.gameHammerBtn.disabled = false;
      }
      els.gameHammerClearBtn.disabled = !hamNow;
    }
  ...
  function updateTeamsLabel() {
    ...
    els.gameTeamsLabel.hidden = false;
    els.gameTeamsLabel.textContent = matchStatusSummary() || "";
  }
  ...
  function setHoleHammer(holeNumber, multiplier) {
    if (multiplier == null) delete roundState.hammers[holeNumber];
      ...
      roundState.hammers[holeNumber] = {
        multiplier: Math.min(MAX_HAMMER, multiplier),
  ...
  function cycleHoleHammer(holeNumber) {
    ...
    else if (cur.multiplier >= MAX_HAMMER) return;
    else setHoleHammer(holeNumber, Math.min(MAX_HAMMER, cur.multiplier * 2));
  }

  function openHammerPad(holeNumber) {
    if (roundState.gameType !== "hammer") return;
    hammerEditHole = holeNumber;
    els.hammerPadTitle.textContent = "Hole " + holeNumber + " hammer";
    var cur = hammerForHole(holeNumber);
    var curMult = cur ? cur.multiplier : null;
    var options = [null, 2, 4, 8, 16, 32, 64];
    els.hammerOptions.innerHTML = "";
    options.forEach(function (mult) {
      var btn = document.createElement("button");
      btn.type = "button";
      btn.className =
        "tee-option" +
        ((mult == null && curMult == null) || mult === curMult
          ? " is-active"
          : "");
      btn.textContent = mult == null ? "Off" : mult + "x";
      btn.addEventListener("click", function () {
        setHoleHammer(holeNumber, mult);
        closeHammerPad();
        haptic(12);
      });
      els.hammerOptions.appendChild(btn);
    });
    els.hammerPad.hidden = false;
  }

  function closeHammerPad() {
    els.hammerPad.hidden = true;
    hammerEditHole = null;
  }
  ...
  function renderGameBoard() {
    ...
    if (showTeams && aCount && bCount) {
      ...
      html +=
        ...
        "</th>";
      if (showContest)
        html += '<th class="contest-col"></th><th class="run-col"></th>';
      html += "</tr>";
    }
    html += "<tr><th>#</th>";
    if (isHammer) html += '<th class="hammer-col-head">Ham</th>';
    ...
    if (showContest) {
      html += '<th class="contest-col">W</th><th class="run-col">±</th>';
    }
    html += "</tr></thead><tbody>";
    ...
    course.holes.forEach(function (h, idx) {
      ...
      if (showContest && showTeams) {
        ...
        if (teamRes.winner) {
          if (teamRes.winner === "tie") {
            ...
            winMark = teamRes.winner;
            winClass = teamRes.winner === "A" ? " won-a" : " won-b";
            if (isHammer && teamRes.stake > 1) winMark += "×" + teamRes.stake;
          }
          if (isHammer) {
            if (teamRes.winner === "A") runA += teamRes.stake;
            else if (teamRes.winner === "B") runB += teamRes.stake;
            runMark = formatLeadText(runA, runB, false, "A", "B");
          } else if (isQuota()) {
            ...
            runMark = formatLeadText(runA, runB, true, "A", "B");
          }
        }
      } else if (showContest) {
        ...
        if (!playerRes.incomplete) {
          if (playerRes.tie) {
            ...
            for (var wi = 0; wi < players.length; wi++) {
              if (players[wi].id === playerRes.winnerIds[0]) {
                wp = players[wi];
                break;
              }
            }
            winMark = wp ? playerInitials(wp) : "W";
            winClass = " won-player";
          ...
          players.forEach(function (p) {
            ...
            if (sc == null) return;
            if (isQuota()) playerRuns[p.id] += holePoints(p, sc, h) || 0;
            else {
              var used = playerUsedScore(p, sc, h);
              if (used != null) playerRuns[p.id] += used;
            }
          });
          runMark = individualLeadMark(players, playerRuns);
        }
      }
      ...
      if (isHammer) {
        html +=
          '<td class="hammer-cell">' +
          '<button type="button" class="hole-hammer-btn' +
          (hHam ? " is-on" : "") +
          '" data-hammer-hole="' +
          h.number +
          '" aria-label="Set hammer for hole ' +
          h.number +
          '">' +
          (hHam
      ...
      players.forEach(function (p, index) {
        ...
        var wonHole =
          (showTeams &&
            ...
            teamRes.winner &&
            teamRes.winner !== "tie" &&
            (p.team === "B" ? "B" : "A") === teamRes.winner) ||
          (!showTeams && winnerIds[p.id]);
        var cell =
          '<td class="score-cell' +
    ...
    if (showContest) html += '<td class="contest-col"></td><td class="run-col"></td>';
    html += "</tr>";
    ...
    if (showTeams && aCount && bCount && showContest) {
      ...
      var finalLead =
        aScore && bScore
          ...
          : "—";
      html += '<tr class="team-match-row"><th>' +
      ...
      html +=
        ...
        "</span></td>";
      html +=
        '<td class="contest-col"></td><td class="run-col team-match-score">' +
        finalLead +
        "</td></tr>";
    }
    html += "</tbody></table>";
  ...
  function toggleHammer() {
    ...
    if (cur && cur.multiplier >= MAX_HAMMER) return;
    cycleHoleHammer(holeNum);
    haptic(16);
  }

  function clearCurrentHammer() {
    if (!course || roundState.gameType !== "hammer") return;
    var holeNum = currentHole().number;
    if (!hammerForHole(holeNum)) return;
    setHoleHammer(holeNum, null);
    haptic(10);
  }
  ...
  els.gameHammerClearBtn.addEventListener("click", clearCurrentHammer);
  els.hammerPadClose.addEventListener("click", closeHammerPad);
  els.hammerPad.addEventListener("click", function (event) {
    if (event.target === els.hammerPad) closeHammerPad();
  });
  els.gameBoard.addEventListener("click", function (event) {
    var selectBtn = event.target.closest("[data-select-hole]");
    if (selectBtn && els.gameBoard.contains(selectBtn)) {
      var selectHole = parseInt(selectBtn.getAttribute("data-select-hole"), 10);
      if (!isNaN(selectHole) && course) {
        for (var i = 0; i < course.holes.length; i++) {
          if (course.holes[i].number === selectHole) {
            holeIndex = i;
            bumpActivity();
            persistRoundLocal();
            haptic(8);
